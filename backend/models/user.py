...
class Role(Document):
    ...
    level: int = Field(...,
                       description="Hierarchy level: 1=Super, 2=Admin, 3=Accountant, 4=Staff")
...
class Permission(Document):
    module: str = Field(...,
                        description="sales, inventory, finance, reports, settings")
    action: str = Field(...,
                        description="create, read, update, delete, approve")
    resource: str = Field(..., description="transaction, user, report, etc.")
    requires_approval: bool = Field(default=False)
    approval_role_level: Optional[int] = Field(
        None, description="Which role level can approve")
    description: Optional[str] = None
    created_at: datetime = Field(default_factory=datetime.utcnow)
...
class UserPermission(Document):
    ...
    granted_by: Optional[str] = Field(
        None, description="User ID who granted this permission")
    granted_at: datetime = Field(default_factory=datetime.utcnow)
    expires_at: Optional[datetime] = Field(
        None, description="Temporal access expiration")
...
class User(Document):
    ...
    created_by: Optional[str] = Field(
        None, description="User ID who created this user")
    created_at: datetime = Field(default_factory=datetime.utcnow)
